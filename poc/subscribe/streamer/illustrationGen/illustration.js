...
export class PoseIllustration {
    constructor(scope, offset = { x: 0, y: 0 }) {
        this.scope = scope;
        this.offset = offset;
        this.frames = [];
        this.skeleton = null; // Ensure skeleton is initialized
        this.skinnedPaths = []; // Ensure skinnedPaths is initialized
    }
    ...
    updateSkeleton(pose, face) {
        ...
        if (this.skeleton) {
            ...
            console.error("Skeleton is not defined.");
        }
        ...
        this.skinnedPaths.forEach(skinnedPath => {
            ...
            skinnedPath.segments.forEach(seg => {
                ...
                seg.point.currentPosition = Skeleton.getCurrentPosition(seg.point).add(this.offset);
                if (seg.handleIn) {
                    seg.handleIn.currentPosition = Skeleton.getCurrentPosition(seg.handleIn).add(this.offset);
                }
                if (seg.handleOut) {
                    seg.handleOut.currentPosition = Skeleton.getCurrentPosition(seg.handleOut).add(this.offset);
                }
            });
    ...
    draw() {
        ...
        if (!this.skeleton.isValid) {
            console.warn("Skeleton is not valid.");
            return;
        }
            ...
            skinnedPath.segments.forEach(seg => {
                ...
                Object.keys(seg.point.skinning).forEach((boneName) => {
                    ...
                    ColorUtils.addRGB(color, 
                        ...
                        drawLine(anchor.add(this.offset), seg.point.currentPosition, {strokeColor: 'blue', strokeWidth: bt.weight});
                });
    ...
    bindGroup(group, skeleton) {
        ...
        // Create secondary bones for the included keypoints.
